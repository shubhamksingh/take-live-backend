...
router.patch('/:id/request', async (req, res) => {
    try {
        ...
        if (!event.pending.includes(req.body.userId)) {
            await event.updateOne({ $push: { pending: req.body.userId } }, { new: true});
            res.status(200).json({message : 'The request has been sent', event : {...event._doc, pending: [...event.pending, req.body.userId]}});
        } else {
